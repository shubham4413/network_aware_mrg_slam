...
private:
    ...
    void cloud_callback( sensor_msgs::msg::PointCloud2::ConstSharedPtr src_cloud_ros )
    {
        ...
        if( !base_link_frame.empty() ) {
            ...
            try {
                ...
                transform = tf_buffer_->lookupTransform( base_link_frame, src_cloud->header.frame_id,
    ...
    {
        rclcpp::Time stamp( static_cast<int64_t>( cloud->header.stamp ) * 1000LL, get_clock()->get_clock_type() );
        if( imu_queue_.empty() ) {
            return cloud;
        }
