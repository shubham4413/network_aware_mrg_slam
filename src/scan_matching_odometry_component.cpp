...
public:
    ...
    ScanMatchingOdometryComponent( const rclcpp::NodeOptions& options ) :
        Node( "scan_matching_odometry_component", options ), consecutive_rejections_( 0 )
    {
        RCLCPP_INFO( get_logger(), "Initializing scan_matching_odometry_component..." );

        initialize_params();

        voxel_grid_filter_        = std::make_shared<pcl::VoxelGrid<PointT>>();
        approx_voxel_grid_filter_ = std::make_shared<pcl::ApproximateVoxelGrid<PointT>>();
        ...
        registration_ = select_registration_method( static_cast<rclcpp::Node*>( this ) );
        ...
        if( get_parameter( "enable_imu_frontend" ).as_bool() ) {
            ...
            msf_pose_sub_ = create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>( "msf_core/pose", rclcpp::QoS( 1 ),
                                                                                                callback_fcn_false );
            ...
            msf_pose_after_update_sub_ = create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>( "msf_core/pose_after_update",
                                                                                                             rclcpp::QoS( 1 ),
                                                                                                             callback_fcn_true );
        }
        ...
        odom_pub_   = create_publisher<nav_msgs::msg::Odometry>( "scan_matching_odometry/odom", rclcpp::QoS( 32 ) );
...
private:
    ...
    pcl::PointCloud<PointT>::ConstPtr downsample( const pcl::PointCloud<PointT>::ConstPtr& cloud ) const
    {
        ...
        double                       downsample_resolution = get_parameter( "downsample_resolution" ).as_double();
        if( downsample_method == "VOXELGRID" ) {
            voxel_grid_filter_->setLeafSize( downsample_resolution, downsample_resolution, downsample_resolution );
            voxel_grid_filter_->setMinimumPointsNumberPerVoxel( get_parameter( "downsample_min_points_per_voxel" ).as_int() );
            voxel_grid_filter_->setInputCloud( cloud );
            voxel_grid_filter_->filter( *filtered );
        } else if( downsample_method == "APPROX_VOXELGRID" ) {
            approx_voxel_grid_filter_->setLeafSize( downsample_resolution, downsample_resolution, downsample_resolution );
            approx_voxel_grid_filter_->setInputCloud( cloud );
            approx_voxel_grid_filter_->filter( *filtered );
        }
    ...
    {
        if( !keyframe_cloud_ ) {
            prev_time_ = rclcpp::Time();
            prev_trans_.setIdentity();
            keyframe_pose_.setIdentity();
            keyframe_stamp_ = stamp;
            keyframe_cloud_ = downsample( cloud );
            registration_->setInputTarget( keyframe_cloud_ );
        ...
        if( get_parameter( "enable_imu_frontend" ).as_bool() ) {
            if( msf_pose_ && rclcpp::Time( msf_pose_->header.stamp ) > keyframe_stamp_ && msf_pose_after_update_
                && rclcpp::Time( msf_pose_after_update_->header.stamp ) > keyframe_stamp_ ) {
            ...
            if( tf_buffer_->canTransform( cloud->header.frame_id, stamp, cloud->header.frame_id, prev_time_, robot_odom_frame_id,
                ...
                try {
                    ...
                    RCLCPP_WARN( get_logger(),
                                 ...
                                 "fixed frame %s: %s",
                                 cloud->header.frame_id.c_str(), stamp.seconds(), cloud->header.frame_id.c_str(), prev_time_.seconds(),
                                 robot_odom_frame_id.c_str(), ex.what() );
                }
            } else if( tf_buffer_->canTransform( cloud->header.frame_id, rclcpp::Time( 0, 0, get_clock()->get_clock_type() ),
                                                 cloud->header.frame_id, prev_time_, robot_odom_frame_id, rclcpp::Duration( 0, 0 ) ) ) {
                try {
                    transform = tf_buffer_->lookupTransform( cloud->header.frame_id, rclcpp::Time( 0, 0, get_clock()->get_clock_type() ),
                                                             cloud->header.frame_id, prev_time_, robot_odom_frame_id );
                } catch( const tf2::TransformException& ex ) {
                    RCLCPP_WARN( get_logger(),
                                 "Could not look up transform with target frame %s, target time %9.f, source frame %s, source time %.9f, "
                                 "fixed frame %s: %s",
        ...
        if( get_parameter( "enable_transform_thresholding" ).as_bool() ) {
            ...
            if( dx > max_acceptable_translation || da > max_acceptable_angle ) {
                ...
                if( consecutive_rejections_ > 0 ) {
                    RCLCPP_INFO_STREAM( get_logger(), "Transform accepted after " << consecutive_rejections_ << " rejections" );
                }
                consecutive_rejections_ = 0;
            }
        }
        ...
        auto        keyframe_transform = matrix2transform( stamp, keyframe_pose_, odom_frame_id, keyframe_str );
        tf_broadcaster_->sendTransform( keyframe_transform );
...
private:
    ...
    geometry_msgs::msg::PoseWithCovarianceStamped::ConstSharedPtr msf_pose_;
    geometry_msgs::msg::PoseWithCovarianceStamped::ConstSharedPtr msf_pose_after_update_;

    int                               consecutive_rejections_;  // number of consecutive rejections due to large transform
    ...
    pcl::PointCloud<PointT>::ConstPtr keyframe_cloud_;  // keyframe point cloud

    pcl::VoxelGrid<PointT>::Ptr            voxel_grid_filter_;
    pcl::ApproximateVoxelGrid<PointT>::Ptr approx_voxel_grid_filter_;
